...
#
# Purpose:
#   Import data from *.csv file with reg codes generated by ipedia_gen_reg_codes.py
#   into a database so that the server can properly authenticate users.

import sys,os,MySQLdb,_mysql_exceptions
import ipedia_gen_reg_codes

g_conn = MySQLdb.Connect(host='localhost', user='root', passwd='', db='ipedia_manage')
g_cur = g_conn.cursor()
...
def main():
    global g_conn, g_cur
    codes = ipedia_gen_reg_codes.readPreviousCodes()
    ...
    for reg_code in codes.keys():
        ...
        assert purpose in ["h","hsm","hppc","pg","es","sn","pog","mr", "test"] or "s:" == purpose[:2]
