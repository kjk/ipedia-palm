...
# Purpose:
#
# Create usage statistics for iPedia.
#
...
DB_HOST = "ipedia.arslexis.com"
DB_PWD = "gaz32orb"
DB_USER = "ipedia-logs"
DB_NAME = "ipedia_manage"

DATA_FILE_NAME = "ipedia-stats.dat"
...
def getRegCodeLastLogId():
    ...
    for row in g_tableVerifyRegCodeLogData:
        requestId = int(row[VRCL_REQUEST_ID])
        if requestId > lastRequestId:
            lastRequestId = requestId
    return lastRequestId
...
def getTableRequestLog():
    ...
    sql = "SELECT request_id, user_id, client_ip, log_date, request_type, search_data, search_result, error FROM request_log WHERE request_id > %s;" % str(lastRequestId)
    #print "sql = %s" % sql
    conn = getConnection()
    cursor = conn.cursor()
    cursor.execute(sql)
...
def getTableVerifyRegCodeLog():
    ...
    sql = "SELECT request_id, user_id, client_ip, log_date, reg_code, reg_code_valid_p FROM verify_reg_code_log WHERE request_id > %s;" % str(lastLogId)
    #print "sql = %s" % sql
    conn = getConnection()
    cursor = conn.cursor()
    cursor.execute(sql)
